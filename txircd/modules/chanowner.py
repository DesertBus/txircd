...
class OwnerMode(Mode):
	def checkSet(self, user, target, param):
		...
		if target.name not in oppingUser.channels:
			return [False, param]
		if "q" in oppingUser.status(target.name):
			return [False, param]
		oppingStatus = oppingUser.status(target.name)
		if user.hasAccess(target.name, "q") and (not oppingStatus or (oppingStatus and user.hasAccess(target.name, oppingStatus[0]))):
			return [True, param]
		return [False, param]
	...
	def checkUnset(self, user, target, param):
		...
		if target.name not in deoppingUser.channels:
			return [False, param]
		if "q" not in deoppingUser.status(target.name):
			return [False, param]
		if user.hasAccess(target.name, "q") and user.hasAccess(target.name, deoppingUser.status(target.name)[0]):
			return [True, param]
		return [False, param]
